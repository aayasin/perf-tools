...
# Pre-push commands:
# tests help of metric; version; prompts for sudo password
# tests sys-wide + topdown tree; MEM_Bandwidth in L5
# tests perf -M IpCall & colored TMA, then toplev --drilldown
# prompt for sudo soon after
# tests topdown across-tree tagging; Mispredict
# tests topdown ~overlap in Threshold attribute
# tests Bottlenecks View
# tests build command, perf -e, toplev --nodes; Ports_*
# tests load-latency profile-step + verbose:1
# builds CLTRAMP3D workload for next tests
# tests default profile-steps, track LBR speed
# tests stats module
# tests analyze module
# tests --delay
# tests prof-no-aux command
# tests unfiltered- calibrated-sampling; PEBS, tma group, bottlenecks-view & over-time profile-steps, tar command
# tests sys-wide non-MUX profile-steps
# tests default from another directory, toplev describe
# tests clean command
# tests study script (errors only)
# tests srcline loop stat
# tests tripcount-mean calculation
# tests force cpu option
# tests sampling by instructions
#
# tests default profile-steps (errors only)
# tests setup-all, ideal-IPC
#
# tests default w/ python2 (errors only)

PRE_PUSH_CMDS := \
    "$(DO) version log help -m GFLOPs --tune :msr:1" \
    "$(MAKE) test-mem-bw SHOW=\"grep --color -E '.*<=='\"" \
    "$(MAKE) test-metric SHOW=\"grep --color -E '^|Ret.*<=='\"" \
    "$(DO) log" \
    "$(MAKE) test-bc2 PM=40 SHOW=\"grep --color -E '^|Mispredict'\"" \
    "echo skip: $(MAKE) test-false-sharin" \
    "$(MAKE) test-bottlenecks AP=\"./kernels/cpuid $(CPUIDI)\"" \
    "$(MAKE) test-build SHOW=\"grep --color -E '^|build|DSB|Ports'\"" \
    "rm -f run-mem-bw && $(MAKE) test-mem-bw RERUN='-pm 400 -v1'" \
    "$(MAKE) tramp3d-v4" \
    "$(MAKE) test-default-track-perf" \
    "$(MAKE) test-stats" \
    "$(MAKE) test-analyze" \
    "$(DO) profile -a './workloads/BC.sh 9' -d1 > BC-9.log 2>&1 || $(FAIL)" \
    "$(DO) prof-no-mux -a './workloads/BC.sh 1' -pm 82 && test -f BC-1.$(CPU).stat" \
    ...
    "cp -r test-dir test-dir0; cd test-dir0; ../do.py clean; ls -l" \
    "$(MAKE) -B test-study" \
    "$(MAKE) test-srcline" \
    "$(MAKE) test-tripcount-mean" \
    "$(MAKE) test-forcecpu" \
    "$(MAKE) test-edge-inst" \
    "$(PY3) $(DO) log profile --tune :forgive:0 -pm 10 > .do-forgive.log 2>&1" \
    "$(PY3) $(DO) profile > .do.log 2>&1 || $(FAIL)" \
    "$(DO) setup-all profile --tune :loop-ideal-ipc:1 -pm 300 > .do-ideal-ipc.log 2>&1 || $(FAIL)" \
    "time $(DO) profile -a \"openssl speed rsa2048\" --tune :loops:9 :time:2 > openssl.log 2>&1 || $(FAIL)" \
    "$(PY2) ./do.py profile -v3 > .do-$(PY2).log 2>&1 || $(FAIL)"

pre-push: help
	@i=0; \
    	for cmd in $(PRE_PUSH_CMDS); do \
        	i=$$(($$i+1)); \
        	if ! grep -q "$$i" .prepush_state.cmd 2>/dev/null; then \
            		echo "Running command $$i: $$cmd"; \
			        /bin/sh -c "$$cmd" || exit 1; \
            		echo "$$i" >> .prepush_state.cmd; \
        	fi \
    	done
