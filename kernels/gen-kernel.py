...
ap.add_argument('--disable_asm', default=False, action='store_true', help='disable mapping loop variables to specific registers with asm, uses random registers instead')
args = ap.parse_args()
...
{
    register uint64_t n%s;
    register uint64_t i0%s;
    if (argc<2) {
  ...
  '/* %s\n */'%references.Comments[args.reference].replace('\n', '\n * ') if args.reference else '', paper, 
                         '' if args.disable_asm else ' asm ("r10")', '' if args.disable_asm else ' asm ("r9")'))
for x in vars(args).keys():
  if 'instructions' in x:
    setattr(args, x, itemize(getattr(args, x)))
for inst in [INST_UNIQ, 'addq $0x8,%rsp'] + args.prolog_instructions: asm(inst, spaces=4)
