...
def cpu_pipeline_width(all_widths=None):
  ...
  if all_widths is None:
    ...
    if goldencove() or redwoodcove(): full_width={'dsb':('IDQ.DSB_UOPS',8),'mite':('IDQ.MITE_UOPS',6),'decoders':('INST_DECODED.DECODERS',6),'ms':('IDQ.MS_UOPS',4),'issued':('UOPS_ISSUED.ANY',6),'executed':('UOPS_EXECUTED.CORE',12),'retired':('UOPS_RETIRED.SLOTS',8)}
    return full_width
